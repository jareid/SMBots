...
public class OverUnder implements IRCGame {
	...
	private List<String> s_invalidBetString = (List<String>) Arrays.asList(BLD
			+ VAR + "\"!ou <amount> <choice>\" " + MSG
			+ "You have entered an invalid choice. Please use " + VAR + "over"
			+ MSG + ", " + VAR + "under " + MSG + "or " + VAR + "7");
	...
	public synchronized List<String> processCommand(String[] commands,
		...
		if (command.equalsIgnoreCase("ouroll")) {
			...
			for (Bet bet : openBets) {
				...
				if (bet.isValid()
						&& bet.getUser().equalsIgnoreCase(user.getNick())) {

					List<String> retList = new ArrayList<String>();

					// generate some die rolls
					int d1 = TrueRandom.nextInt(6) + 1;// random.nextInt(6) + 1;
														// // oh lord
					int d2 = TrueRandom.nextInt(6) + 1;// random.nextInt(6) + 1;
					int total = d1 + d2;

					// record the bet now since they have rolled and can't back
					// out ;)
					...
					if (this.doesBetWin(bet, total)) {
						...
						db.addChips(user.getNick(), bet.getProfile(), winnings,
								null);
						db.recordWin(user.getNick());
						db.addTransaction(user.getNick(), bet.getProfile(), 4,
								winnings, this.ID);

						retList.add(BLD + MSG + "Rolling... " + VAR + total
								+ MSG + ". Congratulations on your win " + VAR
								+ user.getNick() + MSG + "!");

					} else {
						// didn't win,
						retList.add(BLD + MSG + "Rolling... " + VAR + total
								+ MSG + ". Better luck next time " + VAR
								+ user.getNick() + MSG + "!");
						...
						if (Math.random() > r) {
							...
							if (this.doesBetWin(bet, total)) {
								...
								if (bet.getChoice().equalsIgnoreCase("7"))
									winnings = 5 * bet.getAmount();
								// they win pay out and add string
								db.addChips(user.getNick(), bet.getProfile(),
										winnings, null);
								db.addTransaction(user.getNick(),
										bet.getProfile(), 4, winnings, this.ID);

								retList.add(BLD + MSG
										+ "BONUS ROLL! Rolling... " + VAR
										+ total + MSG
										+ ". Congratulations on your win "
										+ VAR + user.getNick() + MSG + "!");
							} else {
					...
					// Generate "rake"
					Rake.getRake(bet.getUser(), bet.getAmount(), ProfileType.fromString(bet.getProfile()));
					
					//check if jackpot won
					if (Rake.checkJackpot()) {
						ArrayList<String> winners = new ArrayList<String>();
						winners.add(bet.getUser());
						Rake.jackpotWon(ProfileType.fromString(bet.getProfile()),
										GamesType.OVER_UNDER, winners, bot, null);
					}
					return (List<String>) retList;
				}
			}

			return (List<String>) Arrays.asList(BLD + MSG + user.getNick()
					+ ": No bet found for you, ");
		} else if (command.equalsIgnoreCase("ou")) {
			// make sure they don't have an open bet otherwise let them know to
			// roll or cancel.

			if (commands.length < 3) // if they have done "!ou" with nothing
										// else
			{
			...
			for (Bet bet : openBets) {
				if (bet.getUser().equalsIgnoreCase(user.getNick())) {
					...
					return (List<String>) Arrays.asList(BLD + VAR
							+ user.getNick() + MSG
							+ ": You already have an open bet, type " + VAR
							+ "!ouroll" + MSG + " to roll!");
				}
			}
			...
			if (choice.equalsIgnoreCase("over")
				...
				if (db_new.checkCredits(user.getNick()) < amount) {
					...
					Bet bet = new Bet(user.getNick(), db.getActiveProfile(user
							.getNick()), amount, choice);
					openBets.add(bet);
					db.removeChips(user.getNick(),
							db.getActiveProfile(user.getNick()), amount);
					db.addBet(bet, 1); // add to table for refunds
					db.addTransaction(user.getNick(),
							db.getActiveProfile(user.getNick()), 1,
							-bet.getAmount(), this.ID);
					return (List<String>) Arrays.asList(BLD + VAR
							+ user.getNick() + MSG + " has bet " + VAR + amount
							+ MSG + " on " + VAR + choice + MSG + "! " + VAR
							+ user.getNick() + MSG + " type " + VAR + "!ouroll"
							+ MSG + " to roll");
				}
			} else {
			...
			for (Bet bet : openBets) {
				...
				if (bet.isValid()
					...
					db.addChips(bet.getUser(), bet.getProfile(),
							bet.getAmount(), null);
					db.addTransaction(user.getNick(), bet.getProfile(), 3,
							bet.getAmount(), this.ID);
