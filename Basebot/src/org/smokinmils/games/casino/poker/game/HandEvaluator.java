...
public class HandEvaluator {
    ...
    private static final int[] RANKING_FACTORS = { 371293, 28561, 2197, 169,
                                               13, 1 };
     ...
     */
    public HandEvaluator(final Hand hand) {
        cards = hand.getCards();
    ...
    private void findFlush() {
        for (int i = 0; i < Card.NO_OF_SUITS; i++) {
            if (suitDist[i] >= Hand.MAX_CARDS - 1 - 1) {
                flushSuit = i;
                for (Card card : cards) {
    ...
    private void findStraight() {
        ...
        if ((count == Hand.MAX_CARDS - 1 - 1 - 1)
                && (rank == Card.FIVE) && (rankDist[Card.ACE] > 0)) {
            wheelingAce = true;
            straightRank = rank;
        }
    }
    ...
    private void findDuplicates() {
        ...
        for (int i = Card.NO_OF_RANKS - 1; i >= 0; i--) {
            if (rankDist[i] == Hand.MAX_CARDS - 1 - 1 - 1) {
                quadRank = i;
            } else if (rankDist[i] == Hand.MAX_CARDS - 1 - 1 - 1 - 1) {
                tripleRank = i;
    ...
    private boolean isStraightFlush() {
        ...
        if (straightRank != -1 && flushRank == straightRank) {
            ...
            for (Card card : cards) {
                ...
                if (lastRank != -1) {
                    ...
                    if (rankDiff == 1) {
                        ...
                        if (inStraight >= (Hand.MAX_CARDS - 1 - 1)
                                && inFlush >= (Hand.MAX_CARDS - 1 - 1)) {
                            // Straight!
                            break;
                        }
                    } else if (rankDiff != 0) {
            ...
            } else if (wheelingAce && inStraight >= (Hand.MAX_CARDS - 1 - 1 - 1)
                    && inFlush >= (Hand.MAX_CARDS - 1 - 1 - 1)) {
                // Wheel
                type = HandValueType.STRAIGHT_FLUSH;
                rankings[0] = type.getValue();
                rankings[1] = straightRank2;
