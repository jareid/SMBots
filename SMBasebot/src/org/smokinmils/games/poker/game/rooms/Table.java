 ...
 */ 
package org.smokinmils.games.poker.game.rooms;
...
import org.smokinmils.games.poker.Client;
import org.smokinmils.games.poker.Utils;
import org.smokinmils.games.poker.enums.ActionType;
import org.smokinmils.games.poker.enums.CommandType;
import org.smokinmils.games.poker.enums.RoomType;
import org.smokinmils.games.poker.game.Card;
import org.smokinmils.games.poker.game.Deck;
import org.smokinmils.games.poker.game.Hand;
import org.smokinmils.games.poker.game.HandValue;
import org.smokinmils.games.poker.game.Player;
import org.smokinmils.games.poker.game.SidePot;
import org.smokinmils.games.poker.tasks.TableTask;
	...
	*/	
    		...
    		if (side.getPlayers().size() > 1) {
                ...
                for (HandValue handValue : rankedPlayers.keySet()) {
                    ...
                    if (winners.size() == 1) {
                    	...
                    	try {
	                    	if (i == 0) {
	                            DB.getInstance().setHandWinner(handID, winner.getName(), potsize);
	                    	} else {
	                            DB.getInstance().addHandWinner(handID, winner.getName(), potsize);                    		
	                    	}
	        	        } catch (Exception e) {
	        	        	EventLog.log(e, "Table", "nextHand");
	        	        }
                        ...
                        for (Player winner : winners) {
                            ...
                            try {
	                            if (i == 0 && y == 0) {
	                                DB.getInstance().setHandWinner(handID, winner.getName(), potShare);
	                        	} else {
	                                DB.getInstance().addHandWinner(handID, winner.getName(), potShare);                    		
	                        	}
                	        } catch (Exception e) {
                	        	EventLog.log(e, "Table", "nextHand");
                	        }
    ...
    private synchronized void nextHand() {
        ...
        for (Player player : players) {
            ...
            try {
            	DB.getInstance().addPokerTableCount(player.getName(), tableID, Profile, player.getChips());
	        } catch (Exception e) {
	        	EventLog.log(e, "Table", "nextHand");
	        }
        }
